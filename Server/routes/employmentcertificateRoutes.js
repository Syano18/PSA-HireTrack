...
const drawWatermark = () => {
    ...
    const drawHeader = () => {
        try {
            const psaLogoPath = path.join(__dirname, '../assets/logo.png');
            const kalogo = path.join(__dirname, '../assets/text.png');
            const bagoLogoPath = path.join(__dirname, '../assets/Bagong.png');            doc.image(psaLogoPath, leftMargin, 20, { width: 70 });
            doc.image(kalogo, leftMargin + 70, 20, { width: 300 });
...
    };
    ...
    doc.on('pageAdded', () => {
        drawWatermark();  // Add watermark first
        drawHeader();
        drawFooter();
        doc.y = 160;
    });    drawHeader();
