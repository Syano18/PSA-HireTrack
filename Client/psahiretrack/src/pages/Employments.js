        ...
        <table className="min-w-full text-sm leading-normal">
          <thead>
            <tr className="sticky top-0 border-b-2 border-gray-200 bg-gray-50 dark:border-gray-700 dark:bg-gray-900/50">
              ...
              <th className="px-5 py-3.5 text-left"><button onClick={() => requestSort('contract_end_date')} className="font-semibold flex items-center uppercase">End Date {getSortIcon('contract_end_date')}</button></th>
          ...
          <tbody>
            {paginatedSpreadsheetList.length > 0 ? paginatedSpreadsheetList.map(rec => {
              const canEditRecord = userPermissions.canManage || userPermissions.isHrDesignate || (userPermissions.isFocalPerson && rec.focal_person_id === sessionState.user.id);
              ...
              return (
                <tr key={rec.id} className="transition-colors duration-200 ease-in-out border-b border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/50">
                  ...
                  <td className="px-5 py-4 text-gray-700 dark:text-gray-300">{rec.contract_end_date ? format(parseISO(rec.contract_end_date), 'MM/dd/yyyy') : 'N/A'}</td>
                  ...
                  <td className="px-5 py-4">
                    <div className="flex items-center justify-center space-x-3">
                      ...
                      {canEditRecord && <button onClick={() => handleEditClick(rec)} className="text-indigo-600 hover:text-indigo-900">Edit</button>}
            ...
            <form onSubmit={handleFormSubmit} id="employmentForm" className="flex-auto p-6 overflow-y-auto">
              ...
              <div className="grid grid-cols-1 gap-x-6 gap-y-4 md:grid-cols-2">
                ...
                <div className="md:col-span-2">
                  ...
                  <textarea id="remarks" name="remarks" value={formData.remarks || ''} onChange={handleInputChange} rows="3" disabled={userPermissions.isHrDesignate} className="mt-1 block w-full p-2 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 disabled:opacity-50"></textarea>
                </div>
              </div>
            </form>
